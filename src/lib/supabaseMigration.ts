...
export const LOW_LATENCY_MIGRATION_SQL = `
...
returns setof public.ai_usage_queue language sql security definer set search_path = public as $fn$
...
export async function applySupabaseMigration(): Promise<MigrationResult> {
  ...
  try {
    ...
    return { applied: false, reason: "migration_failed", message: error instanceof Error ? error.message.slice(0, 300) : String(error) };
  } finally {
